...
| [`Analisis-Nomenclatura-ValueObjects.md`](Analisis-Nomenclatura-ValueObjects.md) | **Puntos N1-N21.** Revisión nombre a nombre de los **30 tipos públicos** de `MoralesLarios.OOFP.ValueObjects`: erratas (`Lenght`), colisiones con la BCL (`Void`, `Empty`, `Endpoint`, `Key`, `Name`), la familia `StringAs*` y sus 44 `CS0108`, y propuesta de unificación con `INumber<T>` | Antes de tocar nombres en `ValueObjects` |
